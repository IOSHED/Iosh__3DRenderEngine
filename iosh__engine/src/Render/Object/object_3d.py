...
from src.Render.Matrix_functions.object_matrix import Object
from src.settings import SPEED_TRANSLATE_X, SPEED_TRANSLATE_Z, SPEED_TRANSLATE_Y, SPEED_TRANSLATE_OBJECT, \
...
class Object3D:
    ...
    def draw_vertices(self, vertices):
        if self.drawing_vertices:
            for vertex in vertices:
                if not Object3D.concatenate_matrix(vertex, self.render.middle_width, self.render.middle_height):
    ...
    def draw_polygons(self, vertices):
        for index, color_face in enumerate(self.color_faces):
            ...
            if not Object3D.concatenate_matrix(polygon, self.render.window.middle_width,
    ...
    def update_position(self, position):
        for mass in range(0, np.shape(self.vertices)[0]):
            self.vertices[mass][0] += position[0]
            self.vertices[mass][1] += position[1]
            self.vertices[mass][2] += position[2]
